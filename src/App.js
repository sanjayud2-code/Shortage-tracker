...
const emptySOForm = () => ({
  soRef: "", projectName: "", dispatchDate: "", dcNumber: "", transport: "",
  parts: [emptyPart()],
});
...
export default function App() {
  ...
  // ── Exports ────────────────────────────────────────────────────
  const exportCSV = () => {
    const hdrs = ["SO Ref","Project Name","Part Number","Item Description","Qty","Dispatch Date","DC Number","Transport"];
    const rows = [];
    sos.forEach(so => {
      so.parts.forEach(p => {
        rows.push(
          [so.soRef, so.projectName, p.partNumber, p.itemDescription, p.quantity,
...
<table><thead><tr>
  <th>SO Ref</th><th>Project Name</th><th>Part Number</th>
                  ...
                  return (
                    <div key={so.id} style={{ background:"rgba(255,255,255,0.04)", border:"1px solid rgba(255,255,255,0.1)", borderRadius:12, overflow:"hidden" }}>
                      <div style={{ display:"flex", alignItems:"center", gap:10, padding:"12px 16px", cursor:"pointer", background:open?"rgba(255,210,0,0.05)":"transparent" }}
                        ...
                        <div style={{ flex:1, display:"grid", gridTemplateColumns:"1fr 1.5fr 1fr 1fr 1fr", gap:8, alignItems:"center" }}>
                          {[
